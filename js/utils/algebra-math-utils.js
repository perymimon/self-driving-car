...
export function getShortestPath(start, end, graph) {
    ...
    while (!end.visited) {
        let segs = graph.getSegmentsLeavingFromPoint(currentPoint)
        for (let seg of segs) {
            let otherPoint = seg.p1.equal(currentPoint) ? seg.p2 : seg.p1
        ...
        let unvisited = graph.points.filter(p => p.visited == false)
        currentPoint = unvisited.reduce((p1, p2) => p2.dist < p1.dist ? p2 : p1)
    }
    let path = []
    currentPoint = end
    while (currentPoint) {
